/*
* LearnOpenGL Tutorial - Getting Started > Coordinate Systems > Exercise 3
* https://learnopengl.com/Getting-started/Coordinate-Systems
* Try to make every 3rd container (including the 1st) rotate over time, while leaving the
* other containers static using just the model matrix.
*/
#include <cstdio>
#include <glad/glad.h>
...
int main(int argc, char *argv[]) {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for (int i = 0; i < CUBE_COUNT; i++) {
			float rotation = glm::radians(20.0f * i); // Initialize rotation.
			
			// Increment every third cube's rotation over time.
			if (i % 3 == 0)
				rotation += static_cast<float>(glfwGetTime());
